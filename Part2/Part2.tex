...
P. E. Sabine écrit en 1932 "\textit{Acoustics and architecture}". Il y reprend les principes de W. Sabine, qui décrivait 20 ans plus tôt, des protocoles de test pour mesurer destemps de réverbérations dans les salle. Pour P. E. Sabine, le modèle de la réverbération est purement statistique. De son point de vue, la densité d'échos à prendre en compte est suffisamment importante pour considérer le phénomène comme pseudo-aléatoire. \cite{Kandelman}. Il suppose ainsi que l’énergie sonore et le temps de réverbération sont uniformes en tout point de la salle. Ces considérations permettent d'exprimer ces deux valeurs l'une par rapport à l'autre comme par exemple dans la formule dite "de Sabine" : 
...
La méthode de résolution exacte utilise le principe des éléments finis ou des éléments aux frontières. Il s'agit de mailler le domaine d'étude par des petits éléments linéiques (2D) ou surfaciques (3D). Sur chacun de ces éléments, on pourra calculer la pression acoustique par résolution de l'équation d'onde (modèle de Green ou Helmotz-Kirchhoff) en considérant les conditions aux limites. Cependant, même si l'implémentation mathématique ne présente pas de difficulté majeure, le problème se complique nettement selon la géométrie de la salle. Effectivement, la taille et donc le nombre d'éléments, dépendra de la longueur d'onde \cite{beamtracing}. Dans un cas comme le théâtre d'Orange où les longueurs se comptent en dizaines de mètres et les fréquences en kilo-Hertz (fréquences audibles), il faudra des mailles de l'ordre du millimètre, et donc des milliard d'éléments. Ce genre de problème est aujourd'hui quasi-impossible à mettre en place de part la puissance de calcul et l'espace mémoire nécessaire. Par ailleurs la création du maillage conforme de ce type représente une difficulté à part entière.
...
\begin{subfigureth}{0.35\textwidth}
	...
	\caption{Représentation de l'angle solide d'un cône de révolution \citep{angle_solide}}
	\label{angle_solide}
	\end{subfigureth}
\end{figureth}
...
Dans cette partie, nous allons détailler l'architecture logiciel et le développement de l'outil de calcul acoustique. Tout d'abord, présentons le cahier des charges :
\begin{itemize}
...
Afin de répondre à ces contraintes, nous avons opté pour certains choix technologiques. Premièrement, le code est développé en C++ pour des raisons de rapidité de calcul. Il s'agit d'un exécutable "client" paramètrable directement depuis l'interface Blender. Au lancement du programme, Blender exporte le maillage au format .obj et le logiciel "client" traite les données à partir de ce fichier.Ainsi, l'outil acoustique est transparent pour l'utilisateur et l'accès au maillage est donc immédiat. On notera par contre que l'outil n'est pas exclusif et qu'il pourra travailler avec n'importe quel maillage au format .obj, qui est un des formats les plus courants, disponible sur la plupart des logiciels de \gls{cao}. 
...
\section{Traitement du maillage}
Comme évoqué précedemment, l'outil traite les données d'un fichier .obj. Pour facilter la gestion du maillages, on s'assurera de n'avoir que des faces triangulaires. Le \gls{modifier} "triangulate" de Blender fait très bien cela. Dans ce fichier, chaque objet est différentié par un en-tête comprenant son nom. S'en suit les coordonnées de l'ensemble de ses vertices, ses normales et de ses textures. Ensuite sont regroupé par materiaux les faces qui sont la combianaisons de trois vertices, une texture et une normale. 
Les sources et récepteurs sont positionnés dans Blender. Il suffit d'utiliser les objets volumiques et les les nommer respectivement "source" et "listener". L'outil de calcul déterminera alors le centre des objets sources et recepteurs en calculant la moyenne des coordonnée des points ainsi que le rayon du recepteur grace à la distance entre son centre et une extremité.
